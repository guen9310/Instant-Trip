...
OTP 인증 단계는 별도 화면이 아니라 `SignInForm` 내부에서 이메일 입력 → 인증 코드 입력으로 전환되는 2단계 흐름입니다. 예전엔 `VerifyForm`/`/sign-in/verify`라는 별도 화면으로 설계됐으나 실제로는 그쪽으로 이동하는 경로가 없어 죽은 라우트였고, 정리하면서 이 표에서도 제거했습니다.
